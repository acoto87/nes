...
static void
nk_gdip_draw_image(GpGraphics* destGraphics, short x, short y, unsigned short w, unsigned short h, void *pixels)
{
    ...
    GdipCreateFromHDC(srcDC, &srcGraphics);
    
    GpImage *img;
    GdipCreateBitmapFromGraphics(w, h, srcGraphics, &img);
    
    GdipDrawImageI(destGraphics, img, x, y);
    
    GdipDeleteGraphics(srcGraphics);
...
nk_gdip_render(enum nk_anti_aliasing AA, struct nk_color clear)
{
    ...
    nk_foreach(cmd, &gdip.ctx)
    {
        ...
        case NK_COMMAND_IMAGE: {
            ...
            nk_gdip_draw_image(gdip.memory, q->x, q->y, q->w, q->h, q->img.handle.ptr);
        } break;
