    ...
    struct nk_rect bounds, nk_flags flags)
{
    ...
    if (!win) {
        ...
        // NOTE(acoto87): this line of code wheren't in the original code, 
        // I added it to allow update existing windows positions and sizes.
        // 
        // But it doesn't work because with it the windows can't be moved with the mouse.
        // win->bounds = bounds;
...
nk_tooltip_begin(struct nk_context *ctx, float width, enum nk_tooltip_position position, float height)
{
    ...
    switch (position)
    {
        ...
        {
            bounds.w = width;
            bounds.h = height > 0 ? height : nk_null_rect.h;
            bounds.x = (in->mouse.pos.x - 1) - win->layout->clip.x - width;
        ...
        case NK_TOOLTIP_POS_UP:
        {
            ...
            bounds.y = (in->mouse.pos.y - 1) - win->layout->clip.y - height;
            break;
        }
...
nk_tooltip(struct nk_context *ctx, const char *text, enum nk_tooltip_position position)
{
    ...
    if (nk_tooltip_begin(ctx, (float)text_width, position, text_height)) {
