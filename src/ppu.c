...
void StepPPU(NES* nes)
{
    ...
    if (renderEnabled) {
        ...
        // NOTE: Review this part to see if it is necessary to respect the timing of the PPU cycles for the sprite
        // evaluation process, since it is possible that some games rely on this behavior. For now, I'll just do the
        // whole process in one cycle.
        ...
        //    from
        //    // the OAM and writing into the secondary OAM as usual, only a signal is active that makes the read always
        //    return $FF.
        //    //
        ...
        //        On even cycles, data is written to secondary OAM(unless secondary OAM is full, in which case it will
        //        read the value in secondary OAM instead)
        //        1. Starting at n = 0, read a sprite's Y-coordinate (OAM[n][0], copying it to the next open slot in
        //        secondary OAM (unless 8 sprites have been found, in which case the write is ignored).
        //            1a.If Y - coordinate is in range, copy remaining bytes of sprite data(OAM[n][1] thru OAM[n][3])
        //            into secondary OAM.
        //        2. Increment n
        //            2a.If n has overflowed back to zero(all 64 sprites evaluated), go to 4
        //            2b.If less than 8 sprites have been found, go to 1
        //            2c.If exactly 8 sprites have been found, disable writes to secondary OAM because it is full.This
        //            causes sprites in back to drop out.
        //        3. Starting at m = 0, evaluate OAM[n][m] as a Y - coordinate.
        //            3a.If the value is in range, set the sprite overflow flag in $2002 and read the next 3 entries of
        //            OAM(incrementing 'm' after each byte and incrementing 'n' when 'm' overflows); if m = 3, increment
        //            n 3b.If the value is not in range, increment n and m(without carry).If n overflows to 0, go to 4;
        //            otherwise go to 3
        ...
        //        4. Attempt(and fail) to copy OAM[n][0] into the next free slot in secondary OAM, and increment
        //        n(repeat until HBLANK is reached)*/
        ...
        //                    // the overflow flag would be set whenever more than 8 sprites were present on the same
        //                    scanline, as expected. ppu->spriteM++;
        //                }
        //            }
        //        }
        //    }
        ...
        //        from secondary OAM 5 - 8 : Read the X - coordinate of the selected sprite from secondary OAM 4
        //        times(while the PPU fetches the sprite tile data) For the first empty sprite slot, this will consist
        //        of sprite #63's Y-coordinate followed by 3 $FF bytes; for subsequent empty sprite slots, this will be
        //        four $FF bytes*/
        ...
        //        Read the first byte in secondary OAM(while the PPU fetches the first two background tiles for the next
        //        scanline)*/
        //}
        // else
        //{
